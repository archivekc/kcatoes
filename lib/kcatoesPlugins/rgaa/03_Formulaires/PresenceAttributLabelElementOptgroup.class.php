  ...
  public function execute()
  {
    ...
    foreach ($nodes as $node)
    {
      ...
      if (strlen(trim($label)) == 0) {
        $this->addResult($node, \Resultat::ECHEC, 'L\'élément n\'a pas d\'attribut
        label ou alors celui-ci est vide');
      }
      else {
        $this->addResult($node, \Resultat::REUSSITE, 'L\'élément a un attribut
        label non vide : '.$label);
      }
    }
  }
}
