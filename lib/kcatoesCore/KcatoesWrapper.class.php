...
class KcatoesWrapper
{
   ...
   */
  public function __construct($testsClass, $htmlContent = null, $url = null)
  {
  ...
  public function output($type = 'csv', $history = false, &$fields = array())
  {
    $this->addLogInfo('Génération de la sortie');
  	...
  	{
  		case 'csv':
  			$outputStr = $this->tester->toCSV();
  			break;
  		case 'json':
  			$outputStr = $this->tester->toJSON();
  			break;
      case 'html':
        $outputStr = $this->tester->toHTML();
        break;
      case 'rich':
	    $outputStr = $this->tester->toRichHTML($history, $fields);
        break;
  		default:
  ...
  private function addLogInfo($infoMessage)
  {
    if ($this->logger instanceof sfLogger)
    {
      ...
      $this->logger->warning($infoMessage);
    }
